/**
 * Resize worker entry — M13 batch 2, ported from pi
 * `coding-agent/src/utils/image-resize-worker.ts`. One request in (transfer
 * list), one response out. The parent (image-resize.ts) terminates the
 * worker after each use — no pooling, no lifecycle beyond a single call.
 */
