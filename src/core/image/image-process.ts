/**
 * Image processing pipeline — M13 batch 2, ported from pi
 * `coding-agent/src/utils/image-process.ts`. The single entry the read
 * tool and @file attachments call:
 *
 *   normalize (whitelist mime; else PNG-convert — BMP, EXIF orientation)
 *   → resize (ladder, unless autoResize=false)
 *   → {ok, data: base64, mimeType, hints[]}
 *
 * `hints` ride the text block next to the image: conversion note and the
 * coordinate-mapping dimension note. Failure is always a message string
 * (pi parity) — the caller decides between "read failed" and "attachment
 * omitted" phrasing.
 */
