...
| **UI contribution** (renderers, `ui.confirm`, autocomplete) | ~~imp has no TUI~~ (stale — the TUI landed in M9). First slice shipped in the task-timer batch: `api.setStatus(key, text)` — one host-styled footer line, no-op headless — as an api member, not pi's per-call `ctx.ui` (the same route `api.confirm` took in M10). See `docs/task-timer-design.md`; further widgets still need their own consumers. |
