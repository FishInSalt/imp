...
- **#compaction-ux 压缩体验修复批(2026-09-24,1158 tests,feat/compaction-ux,设计 docs/compaction-ux-design.md 两轮+实现审查两路)**:用户 dogfood 三报+数据分析追加一项,全部用真实会话 JSONL 实测钉死(压缩条目/retainedTail/逐条 usage/bash 体积分布)。**F1 锚有效性**(核心 bug):estimateContextTokens 锚=最后带 usage 的 assistant,压缩后 retainedTail 的旧 usage(实测 224,852)让 footer 显示 22.5%(真值 2.1%)且 200k 窗假触发压缩。修法四层:buildContext 返回 compactionBoundary(1+tailLen,结构派生);估算器加 minAnchorIndex 参(默认 0 全兼容,锚扫到 floor 为止,锚失效退化字符估算——比 pi 的 "?" 显示更优);runner estimateFloor 在全部 6 个 history 重建点同步(warmup-resume/resume/tree/new 清零/splice);4 个消费点全接(footer、/status、onBeforeTurn 门、子代理 childFloor 局部变量)。设计审查 P0 救命:rev1 把"当前长度"当 floor,resume 后永久禁锚——rev2 改结构派生。实现期自抓:扫描方向初版写反(min(len-1,floor)),child-compaction 测试当场红。**F2 spinner**:ActivitySnapshot.phase 加 "compacting" 字面量,状态机驱动(长操作守卫本就置该态),shell 渲染 "compacting context…";auto 路不推(run 内活动区已占);审查 P2 后文案参数化 compactingLabel(/login 显示 "waiting for login…" 而非撒谎)。**F3 Ctrl+C 中止**:/compact 接 longOpAbort(/tree 先例);compactNow 转发 signal(旧注释引 §7.4"中止持久化半截"已被 abort 质量门推翻——门在 appendCompaction 之前抛);CompactOutcome 加 "aborted";审查 P2 竞态修复:按错误分类(AbortError name)而非 signal.aborted——500 撞 Ctrl+C 不被误标。**F4a 规则 7 后撤回**(§10,用户拍板):"Keep tool outputs small" 是效率建议混进行为契约区,动机证据在 1M 窗订阅制下零实际损害;**F4b 截断 tip 保留**(bash 截断注记教 head/tail/收窄 grep)。审查:设计 needs-fixes(1 P0+5 P1 全核修);实现两路——第一次委派撞 40 轮上限无产出(子代理轮次问题的现场例证),拆两路后 F1 路 approve(3 P3)、F2-F4 路 needs-fixes(3 P2 全修:abort 竞态/login 文案/e2e 空转断言——clear 断言改为 mark 前置+spinner 帧字符钉防静态 note 混淆)。测试 1148→1158(+10):floor 语义 4 钉/store 边界 2/假触发消失(设计测试④审查轮补)/shell 行/e2e/rule-7(后反转)。门禁全绿+build+全局安装。教训:**10% 增长最初当 bug 查、查明是正常累积后又反手当优化项——"没有造成损害的行为"不是问题,证据误读两跳**。

- **#subagent-softlanding 子代理软着陆(2026-09-24,1173 tests,feat/subagent-softlanding,设计四轮+实现审查,docs/subagent-softlanding-design.md)**:事故驱动(2026-09-23 审查子代理 40 轮撞限零文本,44.9k token 无回报,父代理被"(subagent completed with no output)"误导)。**设计演化链是本批最大资产**:rev1 轮次提醒注入(75%/95%)→被 pi-subagents 实测推翻(同构方案 turnBudget 在 0.59.0 整体移除,CHANGELOG :454);rev2 工具封锁(soft20 nudge+hard30 封 read/grep/find/ls)→被用户零污染哲学推翻+技术反证(pi 封锁表不含 bash,cat 可旁路,封锁只是强化恳求);rev3 零注入+60 后备墙+失败信息保全(用户拍板,参照 CC 泄漏源码实测:内建 agent 零 maxTurns/零注入/零时钟,防线=工具池角色化);rev4 折入双路审查(A 路:print 模式挂死 P0→模式感知默认钟;B 路:timeoutMs 三方优先级 P0)+第二轮终审(抓到 §4 测试 9 单位矛盾 P1——上一轮折入时自造的)。**最终形态**:CHILD_MAX_TURNS 40→60 后备墙;CHILD_TIMEOUT_MS 删除,defaultChildTimeoutMs()=TTY 时 undefined(哨兵,Infinity 会 RangeError)/非 TTY 60min;schema 加 timeoutMs(Integer≥1000),优先级 args>frontmatter>模式默认;条件化 clock(timeoutMs 才建,三处 timedOut+finally 联动,'timeout' 默认不可达);撞限诚实渲染二形(有 text→wrap-up 标注 isError:false;无 text→session.filePath+args.prompt 200 字符 CJK 摘要+再派发指引);timeout/abort/crash 同获 handoff,timeout 保持 isError:true;completed 旧标记防误并;~300 words 派发教学。实现审查 APPROVE(2 P2+4 P3 全折入:no-clock 测试补 turns===1、worktree 摘要排除钉、handoff 悬冒号、读取时机 doc 对齐);审查者无法跑的机械检查(删除的 expect 行=5,全是旧渲染字符串替换,零覆盖删减)由我补跑确认。测试 1159→1173(+14,含 e2e 路径落盘钉/TTY 哨兵钉/三方优先级/schema 拒绝面/重试中 timeout)。教训:①**测试文件多块搬移必须整文本替换,行号手术连错三次**(括号配平/孤儿断言/多余 closer,git checkout 后原子重写一次过);②设计审查轮与轮之间自己折入时可能自造矛盾(§4 测试 9 单位),下一轮审查的价值正在于此。
- **#live-thinking-toggle(2026-09-24,feat/live-thinking-toggle,docs/live-thinking-toggle.md)**:Ctrl+T thinking 可见性改为可逆呈现操作——对 TUI 转录中**已保留的**每个 thinking 段(含流到一半的)即时切换,不再清屏重放会话;不触发模型响应、不要求 idle、无会话也可用;不动对话历史/会话文件/工具簿记/编辑器/流缓冲;设置持久化保留。设计文档状态行仍为 "proposed design; implementation not started"(实现后未回写状态,流程瑕疵)。测试文件 76→79;账本补记于 2026-09-25(本批未留独立审查记录,合入信息以提交与测试为准)。
- **#steer-user-style(2026-09-24,fix/steer-user-style,单提交 7492417)**:被消费的 steering 消息渲染为完整用户块(此前呈现样式与普通用户输入不一致)。无设计文档;小修,无审查记录。
- **#tool-display 系列四批(2026-09-25,integration/tool-display-output→integration/builtin-display-final,共 5 提交,+~8k 行,测试文件 79→96)**:显示层大迁移——①tool-display-previews:可扩展工具预览+可读终端呈现(设计 APPROVED,对抗审查关闭:事件顺序+净化澄清);②tool-output-integrity:工具输出完整性与诚实截断标注(设计 approved,审查关闭:read 引号路径+预中止搜索处理;状态行注明实现仍需测试与独立代码审查——后续测试文件已落地,代码审查未留痕);③tool-presentation-hooks:呈现钩子(设计 APPROVED:子来源身份+父执行上下文管道显式化);④builtin-tool-presentation/layout/polish/diagnostics 四连批:内建六工具呈现打磨(四份设计文档均标 COMPLETE 含独立设计+实现审查关闭+验收记录)。账本补记于 2026-09-25;#steer 与 #live-thinking 之外的审查结论以各设计文档状态行为准。
- **#web-search-keyless keyless 搜索回退(2026-09-26,1961 tests,feature/web-search-keyless,merge e406353)**:用户问"web_search 是否支持 keyless、能否改回"——历史先有 keyless(6cf8677),repackage(2864519)移除且设计文档明文禁止无凭据回退。恢复契约:无凭据(env 空+config 文件 ENOENT)→`X-Tavily-Access-Mode: keyless`,有 key→仅 Bearer,两头互斥;凭据状态=未设/key/KEYLESS 符号哨兵,任一迁移(含 key 增删换)清缓存+generation++;keyless 的 401/403/429/432/433 归一到单一提示(provider 只文档化自然语言限流指引,无稳定状态码,故为防御性启发式),其余非 2xx 保持模式中立文案;配置存在但非法仍本地 fail-closed 不静默降级。设计经 3 轮独立对抗审查(rev1 8 findings→rev2 2 P1→rev3 PASS+rev3.1 P3)后实施;实施审查 1 轮 NEEDS-FIXES:P1=Symbol 哨兵换成字符串可全绿且键串与哨兵相撞时缓存复用(修复:新增哨兵串候选回归钉子,变异验证——哨兵临时改成字符串时该钉子变红;首版钉子自身因循环残留 config 假绿,已修),P2×2=keyless 提示未钉精确串/未禁 keyed 措辞、config 失败后"状态置未设"不可观测(rev3.2 改为只声明可观测契约)。全量 1961 绿、typecheck/lint/build 干净;扩展经 ~/.imp/extensions/web-search symlink 安装,重启 imp 即生效。教训:【provider 未文档化的错误码分组是启发式,设计必须明说"防御性",否则与"无稳定码"声明自相矛盾;内部哨兵值的类型正确性没有结构性保证时,要用"键串撞哨兵"场景把碰撞后缓存复用变红;无凭据回退的隐私差异是归因(IP/网络元数据 vs 账户身份),设计应显式记录】。
- **#deepseek-null-usage 修复(2026-09-26,1950 tests,fix/deepseek-null-usage)**:dogfood 第二发:/login deepseek → 切模型 → 发消息即崩 "Cannot read properties of null (reading prompt_tokens_details)"。根因:DeepSeek 流式响应的中间块携带 usage:null(字段存在但值为 null,2026-09-26 实测),imp 的守卫只判 undefined;chat-completions 的 usage 解析直接读 null.prompt_tokens_details。修复:!= null 双守卫 + StreamChunk.usage 类型改 null | {...} 让 wire 形状诚实;测试服务器回放真实形状(null 中间块+末尾用量块)并断言缓存扣除数学不变(100-30=70)。教训:【对外部 wire 的"可选字段"类型标注,unknown 端点的现实是三态:缺省/undefined/null——用 != null 而非 !== undefined 守卫,或先跑一次真实流量探针再定类型;两个 deepseek dogfood bug(double-input、null-usage)都属"实现了协议但没跑过真实端点"——下批接入外部 API 时,实施审查前应先用真实 key 冒烟一次】。
- **#login-dialog-double-input 修复(2026-09-26,1950 tests,fix/login-dialog-double-input)**:dogfood 报告 /login 对话框两行 ">"。根因:构造器把 this.input 加进根容器,prompt() 又把同一实例加进 content——Container.render 只遍历 children 数组不查归属,同帧渲染两次。对照 pi(login-dialog.js:33-54)确认正确做法:input 从不在构造期入树,只在 prompt 打开时随取消提示一起进 content;oauth 设备码态根本不该有输入行。顺带修掉根级静态提示行(它出现在所有状态下,包括没有可提交内容的设备码态)和提交后残留的提示(pi 保留该行,本处为有意偏离:提交后无可取消)。补两个钉子:17(api-key 提示态恰好一行 > + 一处提示)、17b(设备码态零 > 行——更强的判别点:content.clear() 清不掉根级副本,旧 bug 在 oauth 流也可见)。轻量复验 APPROVE(真实变异测试:分别还原两个 hunk,两个钉子各自变红)。教训:【组件树挂载审查——"同一实例进两个容器"这类 bug 测试断言查不到(只查文本存在/消失),要么数行数要么渲染快照;设计审查时逐行对照 pi 构造器本可发现,rev8 的 parity 表锚了状态方法却漏了构造器】。
- **#deepseek-provider DeepSeek 官方 API 家族(2026-09-26,1948 tests,feature/deepseek-provider,merge 4df55dd)**:任务"通过 deepseek 官方 api-key 接入"。模板=zai 家族全链(zai.ts/provider/discover/catalog/login/picker);参照=pi deepseek.ts+openai-completions.ts(thinkingFormat deepseek 分支 :914-926、reasoning_content 回放 :1357-1361)+pi.dev 线上目录实测(flash=V4.1 low:"low" 且支持图片,与 pi 本地 json 漂移——按 M14 规则线上为准)。设计 rev1 对抗审查 APPROVE-with-P1:抓到真问题【P1-1:compaction/branch-summary 原样传 this.level,只有主请求做 off→undefined 映射——我按 pi 主路径写的 thinking 分支会在 off 级摘要请求上误开思考】+v4-pro 费率笔误(0.28→实为 1.32)+MODEL_CANDIDATES 遗漏;rev2 折入后实施。实施审查驳回一次(P1=biome unsafe chain + 两处钉子卫生:known: 列表 substring 匹配漏了 deepseek 新成员、zai 交叉断言假绿——历史里没有 assistant 帧导致 filter 恒真;复验时审查者对修复做了真实变异测试:临时去掉 family 门,确认测试会红)。关键语义:【supportsReasoningEffort 的 catalog 映射 deepseek 用 !==false 而 zai 用 ===true——pi 检测默认值两家相反(absent≠false vs absent≠true),一条 parity 表锚点防止未来合并错】;reasoning_content 回放硬编码该字段名(pi 的字段名通用回放是 D5 未来通用化范围);家族级 auth 选项堵住 OPENAI_API_KEY 借给错端点。D2 入账:zai 家族缺 key 时回退 OPENAI_API_KEY 的同构问题留待独立小批。教训:【设计文档里的"零改动"声明(如费率"实现时复核")本身就是未验证假设——审查者实测 pi.dev 抓出笔误;子串断言 toContain 会静默漏新增列表成员,枚举类钉子应精确匹配】。
- **#login-dialog 独占登录对话框(2026-09-26,1932 tests,feature/login-dialog,merge d1f107d)**:dogfood 报告"/login 步骤乱跳"→审计发现逻辑正确但呈现层三缺陷(P1: 15分钟语义 spinner 盖住几秒的 key 输入;P2: 提示与输入框分离)。方案之争:先推荐 10 行小修,用户追问后完整核算——B 方案(对齐 pi LoginDialog)净删 ~80-100 行机器特例换 ~300 行组件,UX 上限更高,选定 B。**设计经 8 轮对抗审查 + 1 轮独立终审 + 1 轮自我审计**:rev1 驳回(互斥 P0/teardown/Ctrl+C/重入),rev2 驳回(dialogOpen 自锁——authorizedStateful 豁免),rev3 驳回(回退路径失守——shell-conditional 删除),rev4 驳回(§2.3 自相矛盾),rev5 驳回(伪代码尾序),rev7 独立终审(2 P2: LineInput 能力缝隙未指定/1000ms 轮询下限的测试计时),rev8 自我审计抓到真 bug:"waiting 替换内容区"是我发明的,pi 实际追加(URL+code 必须全程可见)——7 轮审查全漏,因为都在验证文档自洽而非对照 pi 实际行为;防复发:§2.1.1 parity 表(每个 parity 声明锚定 pi 源码行号)。**实施审查驳回一次**:P0=selector teardown(SIGINT/close)只拆 UI 不取消流——pending prompt 永不 settle、机器被 dialogOpen 锁死;修复=finish() 内调 dialog.teardownNow()(cancel 公开化)+ 回归钉子。P1=测试 14 假绿(重写为 shell 级真实 held-ask)。终审 APPROVE(顺手修了缩进)。交付:login-dialog.ts(174行组件,pi-tui Input/四态/OSC-8 提示行)、shell.ts openLoginDialog(selector 契约+settled 卫+pendingAsks/pendingSelects 排空)、repl.ts dialogOpen 互斥+豁免、commands.ts loginViaDialog(restore-then-status 尾序)、14 项 TUI 测试(粘贴实测/teardown-first/授权豁免回归)。回退路径逐字不变。**过程教训(两条入账)**:(1)两个方案量级差一个数量级时,禁止只核算小方案;(2)parity 声明必须锚参照系源码——审查文档自洽≠审查文档与现实一致。
- **#moonshotai-provider Moonshot/Kimi 官方 API 家族(2026-09-26,1982 tests,feature/moonshotai-provider,merge b9a0889)**:任务"通过 kimi 官方 api-key 接入"。官方平台对照表核实(Kimi Code FAQ):Open Platform 国内=api.moonshot.cn/v1、海外=api.moonshot.ai/v1(用户充值站 platform.kimi.com=国内站→moonshotai-cn);pi 参照=providers/moonshotai{,-cn}.ts+openai-completions.ts 专项(isMoonshot :1592-1662、thinkingFormat deepseek/openai 分支 :914-963、回放 :1312-1361、usage 兜底 :565/:1520);pi.dev 目录两家实测收录四模型(k2.6/k2.7-code/k2.7-code-highspeed/k3)。设计 rev1 对抗审查 NEEDS-FIXES(1×P1+7×P2+4×P3)→rev2 折入+机械复核 PASS;实现期再修 rev2.1:不可关闭模型的 off 经 provider 入口 clamp 上行(k3→reasoning_effort low、k2.7-code→enabled),真实默认=runner:387-390 的 medium 经 clamp——rev1 误引字段初始化行。关键语义:【catalog compat 驱动 wire 风格(deepseek→thinking{type},openai→reasoning_effort,k3 永不发 thinking);supportsEffort 锚点 ===true(检测默认 false,与 deepseek 的 !==false 相反);回放拆分=文本回放家族级+空串填充模型级(k3 目录旗标,deepseek 家族常数,跨切换按当前请求模型);usage rawUsage=chunk.usage??choice.usage+顶层 cached_tokens;sanitizeEntry 白名单必须同步放行新 compat 字段(实现期测试抓到的真 bug)】。实施审查 2 路并行:静态 0×P1/3×P2/5×P3(deepseek 401 文案未钉、测试标题夸大、compat 缺失默认未钉、cache key 不含 baseUrl 等,全部折入 f5a7e20)+变异 16 发 15 中,唯一漏网(discover cn 分支误用 ai 的 key 解析)已补钉并复验(变异下变红)。真机冒烟:两端点无 key curl 均 401(可达+鉴权形状);带 key 冒烟待用户 /login moonshotai-cn 后跑(/tmp/imp-moonshot-smoke.mts 已备)。全量 1982 绿、typecheck/lint/build 干净。教训:【"零改动"与 clamp/默认链路这类跨函数语义,设计阶段要沿调用链逐跳推演(rev1 两处栽在 off→undefined 与入口 clamp 的交互);catalog 结构变更是 typecheck 覆盖不到的隐形同步点,需 round-trip 测试】。
- **#moonshotai-provider 冒烟记录(2026-09-26)**:用户 `/login moonshotai-cn` 后,`/tmp/imp-moonshot-smoke.mts` 四项全 PASS(k2.6 流式 thinking=19 deltas+usage;k2.6 工具调用 tool_call_start;k2.7-code `thinking:{type:enabled}` 实接受;k3 `reasoning_effort=high` 实接受 thinking=39 deltas);真机 imp 端到端(`imp -p ... -m moonshotai-cn/kimi-k3`,2 turns,exit 0)完成 bash 工具回路,footer 显示 cache↓3.6k(顶层 cached_tokens 解析实况验证);`/v1/models` 实测四模型与快照一致(国内站列表无需鉴权)。设计风险 1/2(k2.7-code thinking 形状、k3 effort 接受度)均解除,无需降级。
- **#task-timer 任务计时扩展(2026-09-26,2005 tests,feat/task-timer,merge 1f01509)**:任务"给 imp 做一个统计当前任务耗时的扩展,要 pi task-timer 的 footer 实时效果"。纯扩展做不到(缺 run 开始事件+状态栏通道),走路径 B 改核心。设计两轮对抗审查:rev1 抓到 3B+8S+21nit——最重=task-timer 的 setInterval 不 unref 会在崩溃路径挂死进程退出(print 挂尾、REPL /exit 挂起,cli.ts:616-621 只设 exitCode 不 exit),评审前设计只分析了显示残留、漏了活性;rev2 CLOSED。交付:①run_start 事件(runTurn 入口恰好一次,overflow 重试只重进 runTurnInner 不双发;子代理不发射,与 run_end 顶层对称;崩溃不配对 run_end 文档化——runner.ts 既有契约"ended, not crashed"不改,notify.mjs 早已接受同款不对称);②api.setStatus(key,text)——api 成员而非 pi 的 per-call ctx(api.confirm/M10 已定此路线,且 setInterval 场景根本没有 handler ctx);bucket=origin:name 由 loader 闭包捕获 facts(subscribe 的 open-section 机制只在加载期存活,运行时拿不到);TUI 第二状态行(sanitize→折行→截断→host dim,buffer 存 raw、截断在 application point 拿真实列宽;legacy/print 能力门 no-op);③examples/extensions/task-timer.mjs。代码评审 1B(lint 在最后一次改测试后未复跑——CI 会红)+2S+8N 全修:N1 排序复用 compareCodePoints(移到 registry 共享给 loader,保持 loader→registry 依赖向)、N2 活 bucket 删不存在 key 不再空推、N3 composeStatusLine 单一定义、N7 500 上限改 code-point 截断不劈代理对、N8 sink 调用隔离。print 崩溃 liveness 自动化 e2e(test/cli-run-start.test.ts:无凭据 hermetic,断言 run_start 已发射+进程不被 execFile timeout kill);REPL/TUI 视觉留用户验收(pty 模拟 TTY 输入不可靠,120s 超时)。2005 tests、typecheck/lint 干净。教训入账:【liveness 类问题(unref)单测只能 pin 机制,e2e 才能 pin 行为——设计 §4.6 把进程退出验证委托 dogfood 是对的分层;提交前最后改过任何文件就必须复跑 lint,CI 门禁是最后防线不是第一道】。
- **#readme-refresh 项目文档刷新批(2026-09-26,2005 tests,docs/readme-refresh,merge e9ba199)**:用户要求"全面检查描述项目的文档(README 一类)是否过期,开发文档不算"。方法:把 README 每条可核验声明与实现对照(provider 家族/CLI HELP/命令注册表/键位/发现路径/常量/环境变量);范围=README+docs/skills.md+examples 内嵌说明+package.json,docs/*-design.md 与 PROJECT_PLAN.md 除外。结论 5 过期+5 缺失。①Status 段自 M0(46a7548)后从未更新(Provider 只写 Anthropic、工具六个、无 TUI/扩展/MCP/多 provider)——重写为当前能力总览;②/login 行清单缺 DeepSeek/Moonshot AI/Moonshot AI CN(实现 7 行,commands.ts:821-859);③DeepSeek 在 README 零覆盖(--help 有段,cli.ts:164)——补 bullet(DEEPSEEK_API_KEY、deepseek-v4-pro/flash、DEEPSEEK_BASE_URL、/login deepseek);④子代理预算"40-turn / 30-minute"未跟进 #subagent-softlanding——改 60-turn 后备墙(constants.ts:13)+时钟规则(TTY 无钟 Ctrl+C 兜底/headless 60min/args.timeoutMs>frontmatter>模式默认,constants.ts:23-25);⑤"task tool's description enumerates agents"已被 prompt-audit P8 移到系统提示 <advertised_agents>(task.ts:139)。缺失补:斜杠清单补 /status /copy /name /trust /worktrees(README 从未提过的五个)及 /login /logout /mcp /settings;Usage 加 --help 指路(print 默认 100 轮/TTY 无限);Setup 加 Node 20+;扩展补 api 只读事实 cwd/version/origin 与 .mjs/.js 加载面;pi 链接 pi-mono→earendil-works/pi。新增 LICENSE(MIT)+License 段链接(package.json 与 README 均称 MIT,仓库此前无文件)。docs/skills.md 关键规则核对一致未改。独立复核:全新上下文子代理逐条对照源码判 CLEAN(0 findings);2005 tests、typecheck/lint 干净。教训:【用户可见文档的"状态段"没有测试钉、评审也不看,最容易腐烂(本批 5 处过期里 4 处是历次特性批改完代码没回写 README);特性批收尾应把"README 里被本批推翻的句子"列为交付物的一部分】。
- **#platform-support 平台支持说明(2026-09-26,2005 tests,docs/platform-support,merge 3b591ed)**:用户问"imp 能否在 Windows/Linux 运行"→平台假设审计后用户拍板:先不支持原生 Windows,把现状写进 README。审计结论:【Linux=CI 已证(ubuntu-latest×Node20/24 跑 typecheck/lint/build/test,ci.yml:23-24);macOS=开发平台,全部 dogfood 在此;原生 Windows 有阻断——①bash 工具 spawn("/bin/bash") 写死(bash.ts:106,无 COMSPEC/pwsh 回退,调用失败但错误路径优雅);②MCP spawn 无 .cmd/shell 解析(client.ts:103,m18 设计已记 R4:183);③无 Windows CI;④测试套件未 Windows 化:HOME 注入 26 处(win32 的 os.homedir() 读 USERPROFILE 不认 HOME,沙箱会漏)、symlink 33 处/chmod 12 处仅 3 文件 6 处 skipIf;⑤rg/fd 安装提示只有 brew/apt(grep.ts:53/find.ts:40)。已具备但未实测:pi-tui 自带 win32 x64/arm64 prebuild(win32-console-mode.node)与 console-mode 分支(terminal.js:90/255/265)、剪贴板三平台分支(clipboard-image.ts:287 PowerShell 原生/WSL、clipboard-write.ts:37 clip.exe、shell.ts:1159 Get-Clipboard)、路径全走 homedir()/os.tmpdir()/path.join、0600 权限已 best-effort(auth-store.ts:68-72)、kill 由 Node 在 Windows 映射为终止进程。WSL=Linux 可直接用。】交付:README 新增 Platform support 节(macOS 开发平台/Linux CI 支持/Windows 暂不支持+两条具体阻断+WSL 出路/外部工具 rg·fd 与剪贴板依赖的按平台说明);若后续要做原生 Windows,工作清单已备:W1 bash 平台化+MCP spawn+taskkill 树杀+windowsHide+CI 矩阵;W2 测试 Windows 化(HOME/USERPROFILE、门控);W3 TUI/剪贴板真机验证,合计 3-5 天。审查评估:单节文档、4 条声明全部本会话内 file:line 直接证据(已复核 ci.yml:23-24/bash.ts:106/client.ts:103),判无需独立对抗审查(先例 #activity-working-label/#footer-per-turn);2005 tests、lint 干净。
- **#npm-packaging npm 发布就绪(2026-09-26,2007 tests,release/npm-packaging,merge e5f5de8)**:用户问"如何发布 imp"→审计后落地 P1+P2。【审计:①npm 上 "imp" 已被占(v0.0.1,2022,maintainer karel)→定名 imp-agent(bin 仍 "imp";包名≠命令名,npx 单 bin 回退实测成立);②旧打包是坏的:npm pack 会带 263 文件含 src/test/docs/.imp 而 dist/ 被 .gitignore 排除→装上也起不来;③依赖全公开(pi-tui 0.82.0/photon-node/typebox/yaml)、仓库公开、LICENSE/engines 齐。】改动:package.json 改名+files 白名单(bin,dist,README,LICENSE)+prepare 构建+repository/homepage/bugs/keywords/author;package-lock 同步;.gitignore 加 *.tgz;README Setup 改 npm install -g imp-agent / npx imp-agent 优先、源码路径次之;CI 新增打包冒烟(仅 node 20:pack→tar 清单断言 dist/bin 在且 src/test/docs 不在→装进 RUNNER_TEMP prefix→--help);新增 test/package-metadata.test.ts(钉 format.ts VERSION=package.json version + bin 文件存在)。验证:pack 后 277 文件/487KB(对比旧 263/1.0MB 且无 dist)、临时 prefix 实装后 imp --version=0.1.0/--help exit 0、typecheck/lint(199 文件)/test 2007 全过。独立评审(全新上下文):B=0、S=1——CI 守护 "! grep -qE" 在 bash -e 下失败不中断(POSIX 对 ! 取反命令豁免 set -e),本地复现(坏清单 exit 0);改为显式 if+exit 1 后复验(坏→1/好→0),并把该步骤体原样在本地 bash -e 跑通全链路;N 项同步(残留 tgz 先 rm、README 冗余 build 去掉、步骤体自足 mkdir)。事故:首轮评审子代理卡死 ~17 分钟——它执行 node -e "import('./dist/cli.js')",而该入口模块有顶层 main:import 即启动 CLI 本体(无参+TTY 进 REPL 永久阻塞);遗留孤儿进程(13284 簇,含 zai-mcp-server 子进程)经用户确认后清理;教训:子代理提示词必须禁止执行 CLI/交互命令,所有命令加 timeout 前缀。】遗留:P3 自动发布(release.yml+trusted publishing 绑定+CHANGELOG/RELEASING.md)未做,npm 账号与绑定待用户。
- **#publishing-design 发布设计(2026-09-26,docs/publishing-design,merge ffd8b9d)**:承接 #npm-packaging 的 P3 遗留,交付 docs/publishing-design.md(194 行)。核心决策:D1 包名 imp-agent/命令 imp(偏离路线图 :602 的 scope 包猜测,已记档);D2 引导路径=首版本机 npm publish(2FA)+绑定 trusted publisher 后转 OIDC 免 token(trusted publisher 配置入口在包设置页、包需先存在,循环依赖有社区实证 #176761);D3 tag=vX.Y.Z 触发器+三层版本守卫(format.ts↔package.json 测试钉、release.yml 校验 tag↔version、手工路径 checklist 命令),tag-on-main 校验取祖先语义(允许旧 tag 重跑,禁未并入 main 的 tag);D4 发布闸门=仓库变量 NPM_PUBLISH_ENABLED(未设时绿跑但必须打 ::warning::+job summary,防"绿而无发布"误判);D5 release.yml(publish job Node24/ubuntu-latest,id-token:write+contents:read,显式 npm install -g npm@latest——trusted publishing 要求 npm≥11.5.1;真发布步需 tag ref+变量 true+非 dry_run 全满足,dispatch 最坏只能干跑);D6 GitHub Release 手动(notes 取 CHANGELOG 单源,保 contents:read 最小权限);D7 Keep-a-Changelog 手写,首版只写摘要指 README(防双源腐烂);D8 回滚=优先补丁版,unpublish 仅限 24h 且版本号永久作废。两轮独立评审:第一轮 B×1/S×4/N×6(先修后实施)——B1 npm CLI 版本前置缺失;S1 手工首发布绕过 tag/版本校验(补第三层守卫);S2 静默跳过误判面;S3 tag-on-main 语义含糊+命令缺失;S4 workflow_dispatch 可成第二真发布入口(修复后复核者穷举象限验证堵死);第二轮逐条复核:五条全 CLOSED、四组内部一致性无矛盾,余 4 点措辞/留痕已修(统一 ${TAG#v} 写法、CI 探测注明 --ref main、状态行改准确表述)后标记已审批。遗留(实现批 release/ci-publish):release.yml+CHANGELOG.md+RELEASING.md+ci.yml 冒烟步骤补 name;之后需用户:npm 账号/2FA→手工发 v0.1.0→绑定 trusted publisher→设 NPM_PUBLISH_ENABLED=true。
- **#ci-publish CI 发布实现(2026-09-26,release/ci-publish,merge 27d03e7)**:按已审批设计落地。交付:①release.yml(gate/publish 两 job;gate=ci.yml 同五步门禁+打包冒烟;publish job ubuntu-latest×Node24、id-token:write+contents:read,步骤=checkout(fetch-depth0)→tag-on-main 祖先校验→tag==package.json 校验(非 tag ref 跳过)→npm install -g npm@latest+断言≥11.5.1→npm ci→三分支:真发布(需 tag ref && NPM_PUBLISH_ENABLED=='true' && dry_run!=true)/干跑(dry_run==true 或非 tag ref)/跳过通报(tag ref 且变量非 true 时 ::warning::+job summary);真发布后轮询 registry 可见性并打印 provenance 链接);②CHANGELOG.md(Keep a Changelog,0.1.0 摘要+Unreleased 空节);③RELEASING.md(bootstrap/常规 tag 流程/dry run/排障/为何不用 token);④ci.yml 打包冒烟补 name。自验证:两 workflow YAML 解析;tag-on-main 守卫用悬空 commit 实测拒绝、main tip 通过;tag/版本 v0.1.0 过、v9.9.9 拒、分支跳过;npm 版本断言 11.8.0 过、10.9.0 拒;npm publish --dry-run 本地无登录 exit 0(仅告警,CI 干跑路径成立);lint 199 文件干净。独立评审(全新上下文):B=0;S=3 全为 RELEASING.md 措辞与实际行为不一致(①"re-run the workflow"在 tag push run 上会真发布且在版本已存在时必失败——已写明;②dispatch 段未区分 tag ref 上 dry_run=false 会真发布,及 tag-on-main 守卫的祖先语义——已写明;③bootstrap 未把"核对 publish-skipped warning+job summary"列为显式验收——已补);N 若干已处理(date/传播时长/set -e)。评审者另实跑:18 象限穷举无"非 tag/变量未设却真发布"组合;两处打包冒烟步骤体 diff 为空;gh release create 参数本机支持;npm 官方文档核对表单字段=workflow 文件名。修复为纯文档措辞(release.yml 零改动)自核后合并。遗留(待用户):npm 账号/2FA→按 RELEASING.md bootstrap 发 v0.1.0→绑定 trusted publisher→设 NPM_PUBLISH_ENABLED=true。
- **#activity-working-label 活动行标签(2026-09-25,1918 tests,fix/activity-row-working-label)**:用户提案——工作状态时输入框上方转动状态行 `⠋ thinking…` 改 `⠋ working…`(shell.ts:576)。依据:①语义不准——该行出现时模型在流式/调工具间隙/生成,非字面"思考";②pi 同位置 WorkingStatusIndicator 用 "Working... (esc to abort)"(client-tui-chat.ts:180);③imp 的 ActivitySnapshot phase 本就有 "working" 值(工具活跃时),标签却只叫 thinking。**明确不动**:Ctrl+T 隐藏思考的占位标签 "Thinking..."(render.ts:443 等三处)——那是思考块占位,语义正确,且是 pi defaultHiddenThinkingLabel 对齐点。snapshot phase 值保持 "thinking" 名(契约,改名徒增测试扰动)。钉子 7 处同步(活动行断言/ticking 测试/中断路径 waitUntil/live-thinking checkCommon——后者区分活动行与隐藏块大写 "Thinking..." 两标签)。esc 提示不加(hint 行已有,重复)。审查评估:单点显示字符串改动+钉子机械同步,判无需独立对抗审查。教训:钉子排查不能只 grep 一个测试文件——live-thinking-screen 的 checkCommon 也钉了活动行,首轮全量跑才暴露。
- **#process-backfill 流程补账批(2026-09-25,1899 tests,chore/process-backfill)**:三件事——①补记账本(上述 6 批,含审查留痕核对:10 份设计文档状态行全部核实,发现 2 处流程瑕疵:live-thinking 状态未回写、output-integrity 代码审查未留痕);②清 biome 债 14→0(12 条 useTemplate 全 FIXABLE,--unsafe 自动修,涉 3 个测试文件 185 用例重跑绿);③补抽查——对无审查留痕的最大批 tool-display-previews+tool-output-integrity(ffd643d+2d80f01,f9eb33b 合入)做了回溯性独立实现审查(31 轮子代理,全部 6 项契约+3 项批次风险逐一 file:line 核验):判决 **CLEAN**(0 P0/P1/P2,3 P3 记档:孤儿 input 块标题不一致、toolFolds 会话内无界增长至 clear()[设计明示的取舍]、.gitignore 冗余条目)。亮点:print 字节契约经非-sink 分支逐字节比对确认不变;read/bash 模型可见输出变更为 output-integrity 设计明示授权;净化器覆盖 C1 单字节序列且幂等;截断诚实性双向核验(锚定扫描在 1000 行切片之前,历史 cap 记录重放重定性为 Partial)。账本纪律中断的根因:多会话并行开发,各会话各自直提 main,无人统一记账——后续约定:每批合入 main 前账本条目为 checklist 必查项。
...
- Current installation and credential instructions supersede the historical setup below: see [web-search README](examples/extensions/web-search/README.md).
...
- **#tree-c 树导航打磨收官批(2026-02-09,1138 tests,feat/tree-c,设计 `51be56f`+`48fb5a6`+实现 `2263c68`+审查修复 `d132031`)**:批 A 延后清单收官(用户裁定:label 时间戳不做)。**打开定位**:initialSelectedId ?? leafId,被藏目标沿 rawParent(全树原始父子图,组件构造期一次遍历)上溯最近可见行,未知 id 落末行;窗口改 pi 居中模型(每渲染 start=max(0,min(sel−⌊vis/2⌋,len−vis)),删 scrollOffset)。**翻页**:←/→/PgUp/PgDn(pi-tui 驼峰 KeyId)整页双端夹紧,不碰查询/折叠。**段跳转**:alt+←/→(ctrl 双绑)foldOrUp/unfoldOrDown——**可见祖先收养图**(设计 P1-1:裸 parentId 分组在默认过滤藏掉无文本 assistant 后 down 死停/up 断链;pi recalculateVisualStructure 的移植)+ isFoldable=分支点/根才可折(设计 P1-2,与 f 键 raw 语义并存——imp 自有键记录偏离);已折行 alt+← 跳段不展开。**水平视口**:renderHorizontalViewport 常量级照抄(anchor=prefix+marker 宽,label 归正文;选中行驱动;sliceByColumn 自 pi-tui 转口;末次 truncate 用 pi 的空省略号)。**中止重开**:picker 路径 for(;;) 内导航,aborted→note+同 entry 预选重开(pi:5298-5302),三选 Esc 重开带预选(pi:5243-5246);numbered/legacy 不变;成功渲染抽 renderNavigateSuccess 两路共用;**轮顶重置 summarize/customInstructions**(实现审查 P2:跨轮泄漏——round1 Summarize+中止→round2 No summary 仍摘要;ask 内改 summarize=choice!==0 逐次重算=pi)。**复制**:ctrl+x→entryCopyText(user contentText/assistant 文本块 join/toolResult 逐 result contentText join/summary;trim 空=undefined)经 ctx.copyText 接缝。**审查教训**:设计 2 P1 均为"与 pi 机制或自己测试的矛盾"(收养图、foldable 判定);实现 P1 是调试残留多行续行躲过 DEBUG_TC 行过滤——**清理调试要按语句块不按行**;排障中两个测试红的真因是改测试误伤(branchedEnvWith→branchedEnv 丢 provider),勿急疑生产码。测试 1127→1138。门禁全绿+build。README 键表更新。
- **#tree-b 树导航打磨批(2026-02-09,1127 tests,feat/tree-b,设计 `1a7ead4`+`d051b93`+实现 `fc7d78e`+审查修复 `52b10e3`)**:批 A 遗留打磨的用户选定组合——标签书签+两设置+/fork 合流。**标签**:`L` 键内嵌编辑(Enter 存 trim 空=删/Esc 取消/其余键嵌套吞;大写 L 避开搜索,pi shift+l 结构);提交先改内存后落库(pi 时序);命令侧 getEntry 校验+try/catch(磁盘异常不走 TUI 键路径裸抛)。**过滤五模式**:labeled-only(node.label!==undefined,leaf 恒显)+all(簿记也显);折叠四处清空(切模式/打字/退格/Esc 清查询,pi:1077-1097);状态标签 pi 字面([labeled]/[all])。**设置**:treeFilterMode(五字面,LIVE 读,选择器专用,legacy 编号树不读)+branchSummary.skipPrompt(嵌套,跳三选,与 env=0 正交);面板循环改 SettingEntry.values(pi values 数组先例——硬编码队列对会写非法值被 coerce 静默吞);两 LIVE 键 status 行 "(live)" 而非 "(next session)"。**/fork 合流**:forkSessionAt=async navigateTree(summarize:false) 薄包装(保 userForkPoints 前置防御);**noop 守卫为 user-leaf 放宽**(设计审查 P1:fork 最新未应答消息是主场景,target=leaf 会被判死;/tree 两面先短路故 pi 对齐不变);/fork 补编辑器回填(既有缺口);note 弃 abandoned 计数改 "N messages kept on this branch"。**两轮审查教训**:设计 P1=自己写错 noop 映射(reviewer 逐行对照代码抓出);实现 0 生产 P1 但 3 枚测试钉子空转/名不符实(>=断言两世界都绿、声称测 project 实测 global——比缺测试更隐蔽)。测试 1104→1127(+23,含 4 破改:五模式 Tab、fork note×3)。门禁全绿+build。
- **#tree 会话树导航批(2026-02-08,1105 tests,feat/tree,设计 `b682d1f`+`c5c0295`+实现 `a96c518`+审查修复 `c4be084`)**:把 M10 的平铺 tip 列表 /tree 升级为 pi 形态树导航器。store:getTree()(孤儿/自指当根、子按时间序、栈式)、branchTo(任意 entry|null=resetLeaf,重编辑首条消息;position marker 落盘)、label entry 旁挂格式(不推进 leaf 不进内容链——**有意偏离 pi 的进链+re-chain 形状**,label 文件级全局)。runner:navigateTree 统一替换 switchSessionBranch(严格超集);**摘要集合=splitBranches(targetId)**(设计审查 P1-1:pi 以 target 求公共祖先且不含,重编辑消息不进摘要);**中止=中止整个导航**(signal.aborted 区分,不算 failed,什么都不动;onLongOpAbort /login 先例——abortActive 是 run 控制器在命令态为哑);身份守卫;user 消息目标 parent=当前 leaf 时不动位置仍回 editorText。组件:buildTreeRows 纯函数共享两 shell,**pi 缩进模型照抄**(线性链平铺、分叉才连接符、活动分支优先排序);三过滤模式+搜索+f 折叠;当前 leaf 恒显(绝对规则,强于 pi);treeSelect 走 select() 全套生命周期契约。命令:三选询问(Esc 回树选择器——pick→ask 循环);editorText 回填仅空编辑器否则 note(丢图像时提示 text only);`/tree <n>` 树行序号(tip 序号废弃,~12 例测试随 active-first 序改)。**两轮审查教训**:设计审查 5 P1(摘要集合等价性/null 语义/哑中止通道/D3 自相矛盾/label 形状错误描述 pi)+实现审查 1 P1(f 键吞搜索字母——测试盲区:自写测试恰好避开含 f 的词)+4 P2。测试 +30(1075→1105)。门禁全绿+build。
- **#system-md 系统提示整替批(2026-02-08,1075 tests,feat/system-md,设计 `838ce0e`+`f9f352a`+实现 `801830c`+审查修复 `44ec7a8`)**:SYSTEM.md/APPEND_SYSTEM.md 两文件×两层级(项目 `.imp/` 过 trust 门/全局 `~/.imp/`),每对一个项目优先。**整替删身份/环境/核心规则/工具目录,四件套+扩展 context 区幸存**(cwd 行/上下文 XML/skills/agents 块——pi customPrompt 分支仍附加上下文+skills+cwd 的设计,亲读核实后照抄并外延);append 落主体后、context 前(pi 相对序)。**拒绝 pi 三处**:读失败把路径字符串当提示文(改穿全局层+警示注记)、未信任静默忽略(加 dim note,仅全局接管时)、纯空白生效(改空文件=槽位占住回落默认)。**两轮设计审查要点入档**:P1-1 信任接缝用会话内布尔不重读库("仅本会话信任"返回 true 不落库,重读即丢);P1-2 测试密闭性(开发机真实 ~/.imp/SYSTEM.md 会机器性破坏套件——实现审查升级为 settings-setup.ts 全局 HOME 沙箱,修一类);同题旧草稿 custom-system-prompt-design.md 显式取代(其信任布尔接缝正确已并入,D4 显式驳回其保留 #Environment 论:平台一次 bash 可得,cwd 才必须钉)。**实现审查再闭环**:混对 supersession 逐对追踪(共享旗标+硬编码文案会把"全局 APPEND 接管"误报成"global system prompt active")、D5 warn 半边(unreadableProject 注记)、cwd 钉 options.cwd、isFile 双处守卫(目录遮蔽)、override:"" 真值判断。两个新文件进 trustRequiringResources(M15 settings.json 文件级先例)。测试 +23(1052→1075,含 P1-1 会话信任钉/密闭双向/中途编辑 refresh 实效/混对文案)。门禁全绿+dist 冒烟。
- **#prompt-audit 提示词优化批(2026-02-08,1052 tests,feat/prompt-audit,设计 `d726707`+实现 `b7b401a`+审查修复)**:32 处模型可见文本全量盘点+pi 逐项对照,原则"独立审视优先,pi 仅在其设计经得起推敲时采纳"。**净效果:系统提示 2704→1223 字符(每请求省 ~370 tokens),edit 高频路径模型侧从整段 diff 变一句话,半截摘要永久 checkpoint 洞堵上,CLAUDE.md 互操作补齐**。九项:P1 edit 一句话输出+**display 渲染专用通道**(唯一接口变更——ToolResult.display 仅存在于 tool_end 事件,persistableResult 在两处 push 点剥离,session JSONL/replay 永不见;消费面 render.ts+repl.ts edit fold,审查 P1-1 修正了设计稿的错列);P2 质量门 `max_tokens`(imp 词表,无 pi 的 error/length;分支摘要补 message_end 分支;审查后另加 abort 门);P3 UPDATE 增量摘要(SUMMARY_MARK 头识别→<previous-summary> 包装+新消息-only,空转录护栏 return null;摘要的摘要漂移止住);P4 上下文 5 文件名 per-dir first-match(override>AGENTS>CLAUDE,.MD 变体)+读失败穿透+XML `<project_instructions>` 包装(标题可伪造→标签边界);P5/P6 目录机制(Tool.promptSnippet 路由行,**文案 imp 重写**——pi 的 "Read file contents" 零路由信息,imp 版答"何时选它";#Tools 教学段+#Editing rules 删除,验证条升核心规则 3);P7 MCP 进目录(**supersedes-M18-D6**,D10 显式取代:路由面一致性+预算封顶重开判定;truncateAtWord 100B/条+2KB 总量降级服务器行;manager onToolsChanged→runner.refreshSystemPrompt——组装时机审查 P2-1,握手/run 边界 fire、parked 静默);P8 roster 迁 `<advertised_agents>` 系统块(pi-subagents 数值 16/12KB/512B,无 opt-in 记档;task description 静态化利 schema 缓存;警示行消子代理误导);P9 read 单行超限 sed 兜底。**实现审查闭环**(无 P0/P1 运行时缺陷):P2-1 全局 ~/.imp/AGENTS.md 漏进 5 名回退(目录遮蔽时会加载 ~/.imp/CLAUDE.md)→豁免;P2-2 supersedes-M18-D6 未记账(即本条);P3×5 全修(12KB 预算按渲染后整块计、generic fold 补 display 感知、abort 门、⎿ display 钉、path 属性 escapeXml)。测试 +29(1023→1052)。门禁:全绿+dist 冒烟(1223 字符、目录、降级行)。**拒绝 pi 化记录**:核心规则 5→6 条保留(优于 pi 两条 always-guidelines)、主动性句保留、bash 截断注记保留 imp 版(多教 read 工具);**延后**:SYSTEM.md/APPEND_SYSTEM.md 整替(用户裁定下一批)、opt-in advertise、扩展 promptGuidelines 机制。
- **#overflow-pagination 搭头批：子代理 overflow 恢复 + Anthropic 家族发现翻页（2026-02-07，1023 tests，feat/overflow-pagination，设计 `f7ea7c3`+实现 `57105da`+审查修复 `3d29c58`）**：M5 账本遗留两项对齐缺口。**设计文档首次走新工作约定**（AGENTS.md：设计过审前不实现）——设计 docs/overflow-pagination-design.md，独立审查判 needs-fixes（2 P1+4 P2+5 P3，逐条亲自核实全落入 §6.5）：P1-1 压缩窗口内 abort 被 compactChildHistory 内部 catch 吞掉只落 false——恢复缝重查 `child.signal.aborted` 分流 timeout/aborted（否则超时误报 crash"nothing safe to compact"）；P1-2 非溢出重试错误保留原始 message（对齐 runner.ts:775，不得一律 overflowGuidance 谎称溢出）；P2：`{compacted, error?}` 返回值（区分 summarizer 失败与无可压缩，error 进 guidance cause 槽）、逐页不落缓存（fetchOnce 现状每页成功即落会缓存半截——合并后一次落盘、中途失败整体 null）、重试重置 maxIterations（最坏 2×CHILD_MAX_TURNS，主循环同形记为决策并修订注释）、fetchPage 新缝（fetchOnce 签名容不下翻页字段）。**实现（子代理恢复）**：launchLoop(userMessage) 缝——重试传 undefined（失败尝试已把 prompt 留在 history，loop 只追加非空 prompt）；单次压缩重试对齐主循环与 pi `_overflowRecoveryAttempted`；crashWith/statsFromHistory 辅助——重试成功计账从 historyStats+summarized 累计（两轮真实成本都在 history）；compactChildHistory 返回 `{compacted, error?}`（onBeforeTurn 调用点忽略，行为不变）。**实现（翻页）**：实测定位——camelCase `{data, firstId, hasMore, lastId}` 在用户主链路的 **anthropic 分支**（z.ai 兼容端点 api.z.ai/api/anthropic/v1/models），zai coding 端点 `{object, data}` 无翻页字段；`limit`/`lastId` 参数实测被服务端忽略——"无新 id 即停"去重使忽略游标的服务端自动退化为单页（永不循环）；after_id（Anthropic 规范）+encodeURIComponent；双命名 wrapper（camel+snake）；窗口富集每页仍触发；zai/openai/codex 走 fetchJson 薄包装行为完全不变。**实现审查（无代码缺陷，§6.5+D1-D6 逐项核实落实）**：仅 1 测试级 P2（D3 钉子 `>=1` 不判别——改精确 turns=2/usage 160in-17out）已修；顺手 MODELS_PAGE_CAP 导出消魔数、过时注释 2.5s→4s、补两缓存语义钉（合并列表 TTL 内零重取、中途失败不落缓存下次全量重取）。测试 +14（1009→1023）：恢复 6（重试成功无重复 prompt、二次溢出恰一次 summarizer 真调用、熔断 guidance 零调用、单消息 nothing-to-compact、压缩窗口超时→timeout 不误报——信号感知挂起 summarizer、401 真因透传）+翻页 8（今日 z.ai 形状钉、camel/snake 翻页合并、忽略游标去重停、页上限、缓存两钉、zai 单请求）。门禁：1023/1023、typecheck 0（src+test 双配置）、biome 净、dist 冒烟 2/2。
- **#mcp-tools M18 MCP stdio 工具接入（2026-02-06，1009 tests，feat/mcp-tools，设计 `a1a66b8`+实现 `f59065b`+审查 `7d1c37f`）**：设计 docs/m18-mcp-design.md（D1 原生模块非扩展——imp 无包管理器、扩展路径需先做动态注册 core 改动；D2 直连摊平 `<server>_<tool>`，有意偏离 pi 默认代理模式，触发=≥10 工具服务器；D3 手写 JSON-RPC 2.0 over NDJSON 零运行时依赖；D4 无配置零开销+mcp.enabled 总门+IMP_MCP=0 逃生口；D5 七项延后各记触发条件；D6 不加 MCP 系统提示段——pi promptSnippet=描述截断 100 字符纯冗余）。**四模块**：config（五路径发现 pi-adapter 同序、同名整条替换=记档偏离 pi 字段级合并+URL 凭据绑定（触发=HTTP/凭据服务器）、${VAR}/$env:VAR/{env:VAR} 三形态展开、坏文件/坏条目一行 note 不阻断启动）；client（initialize 宽松版本协商、游标分页页上限 10、45s 连接/120s 调用超时、abort→notifications/cancelled、杂散行容忍、10MB 行防御、stderr 2KB 环形、stdin.end→3s SIGTERM→2s SIGKILL）；bridge（schema 直通+type:object 规范化、文本块拼接+非文本计数省略、isError 直传、MAX_BYTES 尾截、execute 永不抛=bash 契约）；manager（晚到工具 run 边界 flush——loop toolMap 每 run 快照，中途 push 会"模型可见不可执行"；双路重连：启动失败=run 边界重试 ≤3+30s 冷却（之后会话内 failed，恢复=重启），中途掉线=调用触发单飞+同调用透明重试；与 BUILTIN_TOOL_NAMES 冲突跳过——手列表迁 core/constants.ts 单一权威源）。**接线**：settings mcp.enabled 门+/settings 行（envShadow IMP_MCP）、/mcp 命令（allowedDuringRun 同 /status、五态渲染+环境/设置门专行）、repl run 边界+gracefulExit close+forceExit forceKill、cli createMcpSetup 双路径（print 模式 onRunStart/onRunEnd 包唯一 run+close 在 finally）。**测试**：fake server fixture 九模式（ok/two/paginate/badline/neverinit/die/liarcursor/utf8split/blocks；dieonce 跨进程闩锁、pid 见证、cancelled 回执）+e2e（scripted provider 驱动真 loop 调 fake_echo→结果回模型）+runRepl 集成（/mcp 渲染+/exit 杀子进程）。**真机验收**：zai-vision 8 工具就绪、GLM 主模型自主调 analyze_image 识别本地图（主模型无视觉、视觉计算在 MCP 子进程内完成）；R2 钉子兑现（protocolVersion 2025-06-18、zai-mcp-server 0.1.5、capabilities 仅 tools）。README MCP 段（v1 范围+七延后触发表）。
- **#mcp-tools 审查闭环（同日，1009 tests，`7d1c37f`）**：reviewer 判 **BLOCK**（2 P1+5 P2+6 P3，逐条亲自核实全属实）。**P1-1 runRepl→ReplMachine 漏传 mcp**（可选键 tsc 静默）→四处死代码：优雅退出不杀子进程（pty 实测 /exit 挂起——此前真机验收恰好绕过：空闲连上+调用不经边界机，验收盲区）、边界 parking 不生效、启动重连死、/mcp 谎报"无配置"。修复=补传+键改必填可空（漏传=编译错误，结构性防再犯）+mcp-wiring 集成钉。**P1-2 print 失败路径泄漏**：close 只在成功路 try 内——移 finally。**P2**：print 无边界机（晚到握手泄漏进在飞 run→包 onRunStart/End）；逐块 utf-8 解码跨块多字节字符腐坏→整行当杂散行丢→120s 挂起（字节缓冲按 0x0A 切+utf8split 中文跨块钉）；页上限静默截断（listTools 返回 capped+manager 记 note+liarcursor 永假 cursor 钉）；桥接结果无截断违反仓内不变量（MAX_BYTES 尾截）；§7 桥接测试面假阳性（blocks 工具从未进工具表——补 bridge 单测 14 条+cancelled 回执）。**P3**：abort 监听泄漏（settle 时 detach+归零钉）、connect 失败单 SIGTERM→完整优雅序列、/mcp 对 IMP_MCP=0 谎报→专用行、/settings envShadow、显式非 object schema 改整体换新（§4"包一层"原意）、设计文档四处对齐+adapter 源码路径修正（reviewer 找不到是任务书写错路径——实际 /Users/z/.pi/agent/npm/node_modules/pi-mcp-adapter/，三形态 env/字段级合并/promptSnippet 截断亲自复核属实）。真机复验：/exit 带活服务器干净退出+子进程死、imp -p 成功/坏 token 失败两路均退出且子进程死、/mcp 真实渲染 "fake: connected · 2 tools"。门禁：1009/1009（957→991→1009）、typecheck 0、biome 净、dist 冒烟 4/4。
- **#followup-runs 审查闭环（同日，957 tests，`8a1d725`+修复）**：reviewer 判 needs-fixes（1 P1+5 P2，全部亲自核实）。**P1 已修**：followUpMessages 谓词漏 bang 检查——"! cmd"+alt+enter（运行中）会被当作模型 followUp 消费，而同条目活到 flush 时却按 shell 执行：同一输入结果由时序决定，违反"bang 条目 flush-only"契约（flushQueue 按内容判 bang 不看 mode，故修复=drain 谓词补 `!isBangLine`，与 steeringMessages 同构）+端到端钉（bang+alt+enter 混在真 followUp 后：请求永不含 bang 文本、flush 后 shell 真跑出输出、队列行标 bash）。**P2×4 已修**：abort 测试残留 DBG1 调试块（错误引用 env.requests，注入 provider 不记录它——删除）；loop 边界 poll 补 caller 有界不变量注释（maxIterations 不守纯文本延续轮，poll 消费方必须每交付消耗有限队列项，否则永旋无守卫）；队列视觉测试的"区域已清"等待可空窗恒真（先等 ✓ 行真实重绘再断言）；/help、SubmitMode、QueueEntry 三处文案更新到 M17 语义（/help 金样钉随动）。**P2×1 记档不修（既有）**：steering poll 位置在 onBeforeTurn 前，pi 在 prepareNextTurn 后重 poll（长 compaction 期间排队的 steer 晚一轮）——M17 前即有的形状，与本批同缝但独立关切。门禁：957/957 mac、typecheck 0、biome 净、dist 冒烟重跑通过。
