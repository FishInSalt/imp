/**
 * EXIF orientation — M13 batch 2, ported from pi
 * `coding-agent/src/utils/exif-orientation.ts` (JPEG APP1 and WebP EXIF
 * TIFF walks, orientations 1–8). Phone photos carry orientation as
 * metadata; photon decodes the raw pixel array without applying it, so the
 * transform runs before resize/encode — otherwise "portrait" arrives
 * landscape on the wire.
 */

import type { Photon, PhotonImageType } from "./photon.js";
...
/** Read the EXIF orientation tag (1–8; 1 when absent/unparseable). */
...
/** Exposed for tests (parser goldens on crafted TIFF fixtures). */
export const __exifInternals = { readOrientationFromTiff };
...
/**
 * Apply the EXIF orientation in pixel space. Orientation 1 returns the
 * image unchanged; flips mutate in place; rotations return a new image
 * (the caller frees the old one when different).
 */
