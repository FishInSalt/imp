/**
 * Image magic-byte sniffing (M13). Ported byte-for-byte from pi
 * (`packages/agent/src/harness/tools/image.ts`) — detection is by content,
 * never by file extension.
 *
 * Rejected on purpose: JPEG-XR (0xf7 marker after the SOI — no inline API
 * support) and animated PNG (acTL chunk) — same calls pi makes.
 */
